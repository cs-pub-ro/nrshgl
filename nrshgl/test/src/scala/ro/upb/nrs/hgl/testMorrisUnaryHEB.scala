...
class TestAdditionSingleMorrisUnaryHEB extends AnyFlatSpec with ChiselScalatestTester{
    behavior of "Addition on MorrisUnaryHEB"
    val size = 8
    val rounding = RoundEven

    it should "add 2 values" in {
        test(new AdditionOperationModule(() => MorrisUnaryHEB(size, rounding, true))) { c =>
            val nrs : ro.upb.nrs.sl.FixedPrecisionNumberRepresentationSystem = ro.upb.nrs.sl.MorrisUnaryHEB(0.0d, size, ro.upb.nrs.sl.RoundEven)
            val a : Int = 1
			...
			c.clock.step()
            val result_nrs : ro.upb.nrs.sl.MorrisUnaryHEB_B = (value1_nrs + value2_nrs) match {
                case that : ro.upb.nrs.sl.MorrisUnaryHEB_B => that
                case _ => ro.upb.nrs.sl.MorrisUnaryHEB_NR(size, ro.upb.nrs.sl.RoundEven)
            }
...
class TestAdditionExhaustiveMorrisUnaryHEB extends AnyFlatSpec with ChiselScalatestTester{
    behavior of "Addition on MorrisUnaryHEB"
    val size = 8
    val rounding = RoundEven

    it should "exhausting verification +/-" in {
        test(new AdditionOperationModule(() => MorrisUnaryHEB(size, rounding, false))) { c =>
            val nrs : ro.upb.nrs.sl.FixedPrecisionNumberRepresentationSystem = ro.upb.nrs.sl.MorrisUnaryHEB(0.0d, size, ro.upb.nrs.sl.RoundEven)
...
class TestMultiplicationSingleMorrisUnaryHEB extends AnyFlatSpec with ChiselScalatestTester{
    behavior of "Multiplication on MorrisUnaryHEB"
    val size = 8
    val rounding = RoundEven

    it should "multiply 2 values" in {
        test(new MultiplicationOperationModule(() => MorrisUnaryHEB(size, rounding, true))) { c =>
            val nrs : ro.upb.nrs.sl.FixedPrecisionNumberRepresentationSystem = ro.upb.nrs.sl.MorrisUnaryHEB(0.0d, size, ro.upb.nrs.sl.RoundEven)
            val a : Int = 114
            val binary1 = ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodetoBinaryString(ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodeFixedWidth(size, ro.upb.nrs.sl.NaturalNumber(a).binaryEncode))
            val value1_nrs = ro.upb.nrs.benchmark.basicBenchmarkFunctions.fromBinaryString(binary1, nrs)
...
class TestMultiplicationExhaustiveMorrisUnaryHEB extends AnyFlatSpec with ChiselScalatestTester{
    behavior of "Multiplication on MorrisUnaryHEB"
    val size = 8
    val rounding = RoundEven

    it should "exhausting verification *" in {
        test(new MultiplicationOperationModule(() => MorrisUnaryHEB(size, rounding, false))) { c =>
            val nrs : ro.upb.nrs.sl.FixedPrecisionNumberRepresentationSystem = ro.upb.nrs.sl.MorrisUnaryHEB(0.0d, size, ro.upb.nrs.sl.RoundEven)
			for( i <- 0 until (1<<size)) {//
				for( j <- 0 until (1<<size)) {
                    val binary1 = ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodetoBinaryString(ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodeFixedWidth(size, ro.upb.nrs.sl.NaturalNumber(i).binaryEncode))
...
class TestDivisionSingleMorrisUnaryHEB extends AnyFlatSpec with ChiselScalatestTester{
    behavior of "Division on MorrisUnaryHEB"
    val size = 8
    val rounding = RoundEven
    it should "Division 2 values" in {
        test(new DivisionOperationModule(() => MorrisUnaryHEB(size, rounding, true))) { c =>
            val nrs : ro.upb.nrs.sl.FixedPrecisionNumberRepresentationSystem = ro.upb.nrs.sl.MorrisUnaryHEB(0.0d, size, ro.upb.nrs.sl.RoundEven)
            val a : Int = 128
            val binary1 = ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodetoBinaryString(ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodeFixedWidth(size, ro.upb.nrs.sl.NaturalNumber(a).binaryEncode))
            val value1_nrs = ro.upb.nrs.benchmark.basicBenchmarkFunctions.fromBinaryString(binary1, nrs)
...
class TestDivisionExhaustiveMorrisUnaryHEB extends AnyFlatSpec with ChiselScalatestTester{
    behavior of "Division on MorrisUnaryHEB"
    val size = 8
    val rounding = RoundEven

    it should "exhausting verification /" in {
        test(new DivisionOperationModule(() => MorrisUnaryHEB(size, rounding, false))) { c =>
            val nrs : ro.upb.nrs.sl.FixedPrecisionNumberRepresentationSystem = ro.upb.nrs.sl.MorrisUnaryHEB(0.0d, size, ro.upb.nrs.sl.RoundEven)
			for( i <- 0 until (1<<size)) {//
				for( j <- 0 until (1<<size)) {
                    val binary1 = ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodetoBinaryString(ro.upb.nrs.sl.auxiliaryFunctions.BinaryEncodeFixedWidth(size, ro.upb.nrs.sl.NaturalNumber(i).binaryEncode))
